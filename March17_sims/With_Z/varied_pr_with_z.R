...
res = data.frame(rep = 1:num_reps, 
                 ...
                 fpr = rep(c(rep(0.1, times = num_reps / 6),
                           rep(0.25, times = num_reps / 6), 
                           rep(0.5, times = num_reps / 6)), times = 2),
                 tpr = rep(c(rep(0.9, times = num_reps / 6),
                             rep(0.75, times = num_reps / 6),
                             rep(0.5, times = num_reps / 6)), times = 2),
...
for (r in 1:num_reps) {
  # Simulate data 
  z = rnorm(n = res$n[r])
  e = eta_finder(fpr = res$fpr[r], tpr = res$tpr[r]) #real data rates at 1 mile level
