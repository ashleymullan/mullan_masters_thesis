...
vnwz <- read_csv("vnwz.csv")
vnwz <- vnwz |>
  cbind(fpr = rep(0.1, times = nrow(vnwz)),
        tpr = rep(0.9, times = nrow(vnwz)),
        b0 = rep(-2.246, times = nrow(vnwz)),
        b1 = rep(0.155, times = nrow(vnwz)))

vqwz <- read_csv("vqwz.csv") 
vqwz <- vqwz |>
  cbind(fpr = rep(0.1, times = nrow(vqwz)),
        tpr = rep(0.9, times = nrow(vqwz)),
        b0 = rep(-2.246, times = nrow(vqwz)),
        b1 = rep(0.155, times = nrow(vqwz)))

vprwz <- read_csv("vprwz.csv") 
vprwz <- vprwz |>
  cbind(b0 = rep(-2.246, times = nrow(vprwz)),
        b1 = rep(0.155, times = nrow(vprwz)))

vb0wz <- read_csv("vb0wz.csv") 
vb0wz <- vb0wz |>
  cbind(fpr = rep(0.1, times = nrow(vb0wz)),
        tpr = rep(0.9, times = nrow(vb0wz)),
        b1 = rep(0.155, times = nrow(vb0wz)))

vb1wz <- read_csv("vb1wz.csv") 
vb1wz <- vb1wz |>
  cbind(fpr = rep(0.1, times = nrow(vb1wz)),
        tpr = rep(0.9, times = nrow(vb1wz)),
        b0 = rep(-2.246, times = nrow(vb1wz)))

full_results <- rbind(vnwz, vqwz, vprwz, vb0wz, vb1wz)
full_results |> nrow() #94K replicates
full_results |> drop_na() |> nrow() #89217 reps, 4783 dropped
