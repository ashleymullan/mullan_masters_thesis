...
for (r in 1:num_reps) {
  # Simulate data 
  z = rnorm(n = 10000) #rbinom(n = 10000, size = 1, prob = 0.3) ## Z ~ Bern(p = 0.3)
  xstar = rbinom(n = 10000, size = 1, prob = 1 / (1 + exp(- (1 + 2 * z)))) 
  x = rbinom(n = 10000, size = 1, prob = 1 / (1 + exp(-(e[1] + e[2] * xstar + e[3] * z))))
  lambda = exp(b0 + b1 * x + b2 * z) ## mean of the Poisson distribution for Y|X,Z
  y = rpois(n = 10000, lambda = lambda) ## Y|X,Z ~ Pois(lambda), where lambda is a function of X, Z
  q = rbinom(n = 10000, size = 1, prob = 0.75) #0.25) ## queried indicator
  ...
  nlm_res = nlm(f = loglik_mat, 
                p = cc_fit, #c(b0, b1, b2, e), #
                hessian = TRUE, 
                Y_name = "y",
                X_name = "x",
                Z_name = "z",
  ...
  res[r, -1] = c(optim_res$convergence, optim_res$par)
}

res |> colMeans()


#make sims out of this
