...
Sim Worksheet Stuff
```{r}
error_settings <- c(1,2,3)
n_settings <- c("s","m","l")

set.seed(1031) ## need to move the seed outside of the generate_data() function
...
for (i in 1:nrow(results)) {
  ...
  # Must do the same for standard errors 
}

#plot empirical distributions
results |>
  ggplot(aes(x = betahat1)) +
  geom_histogram() +
  geom_vline(xintercept = 2, color = "red") +
  labs(title = "Empirical Mean: 1.56",
       subtitle = "Truth: 2")

results |>
  ggplot(aes(x = betahat1, group = n)) +
  geom_histogram() +
  geom_vline(xintercept = 2, color = "red") +
  facet_wrap(~n) +
  labs(title = "Varying the sample size doesn't fix it.")

results |>
  ggplot(aes(x = betahat1, group = muU)) +
  geom_histogram() +
  geom_vline(xintercept = 2, color = "red") +
  facet_wrap(~muU) +
  labs(title = "Messing with the error level doesn't fix it.")


#show means of parameter estimates
mean(results$betahat1)

#show means of SE estimates
mean(results$se_betahat1)
