...
info <- function(beta, X, Z = NULL, data) {
  ...
  #cat("lambda", round(l, 1), "\n")

  #initialize design matrix
  n <- nrow(data)
  intercept <- rep(1, times = n)
  dm <- cbind(intercept, data)
  #cat("dim dm = ", dim(dm), "\n")
  ...
  #compute and return information
  t(dm) %*% w %*% dm
}
```
...
my_est <- round(find_beta(start_guess = c(0,0), x = x, y = y, z = NULL, data = data.frame(x,z))$estimates,2) 
